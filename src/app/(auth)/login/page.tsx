...
function LoginContent() {
  ...
  useEffect(() => {
    let changed = false;
    if (sp.get("registered") === "1") { toast.success("สมัครสำเร็จแล้ว เข้าสู่ระบบได้เลย"); changed = true; }
    if (sp.get("logout") === "1")     { toast.info("ออกจากระบบเรียบร้อย"); changed = true; }
    const err = sp.get("error");
    if (err) { try { toast.error(decodeURIComponent(err)); } catch { toast.error(err); } changed = true; }
    if (changed) {
      const params = new URLSearchParams(sp.toString());
      params.delete("registered"); params.delete("logout"); params.delete("error");
  ...
  // ---------- REGISTER ----------
  const [reg, setReg] = useState({ username: "", password: "", name: "", signupCode: "" });
  ...
  async function register() {
    ...
    try {
      ...
      if (res.ok && data?.ok) { router.replace("/login?registered=1"); return; }
  ...
  if (tab === "login") {
    return (
      <div className="card p-5 sm:p-6 shadow-sm rounded-2xl">
        ...
        {/* ฟอร์มล็อกอิน (ใช้อินพุตสูงขึ้นสำหรับมือถือ) */}
        <div className="[&_.input]:h-11 [&_.btn]:h-11">
          <LoginForm />
        </div>
